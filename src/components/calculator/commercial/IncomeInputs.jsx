...
export default function IncomeInputs({ data, onChange, language = 'en', propertyData = {} }) {
    ...
    const lastCalculatedRentRef = useRef(null);
    ...
    useEffect(() => {
        const rentableArea = propertyData.rentable_area_m2;
        const propertyType = propertyData.property_type || 'office';
        const currentRent = data.annual_rent;
        ...
        console.log('[IncomeInputs] Checking for calculation', {
            ...
            lastCalculatedRent: lastCalculatedRentRef.current
        });
        ...
        if (!rentableArea || rentableArea <= 0) {
            console.log('[IncomeInputs] No rentable area yet, skipping');
            return;
        }
        
        // Check if we need to recalculate
        const areaChanged = lastCalculatedAreaRef.current !== rentableArea;
        ...
        const calculatedRent = Math.round(rentableArea * monthlyRate * 12);
        
        // Only update if the calculated value is different from current
        if (calculatedRent === currentRent && calculatedRent === lastCalculatedRentRef.current) {
            console.log('[IncomeInputs] Calculated rent matches current rent, skipping update');
            // Update tracking even if we don't update the value
            lastCalculatedAreaRef.current = rentableArea;
        ...
        // Update our tracking refs BEFORE calling onChange
        lastCalculatedAreaRef.current = rentableArea;
        lastCalculatedTypeRef.current = propertyType;
        lastCalculatedRentRef.current = calculatedRent;
        ...
        propertyData.rentable_area_m2,
        propertyData.property_type,
        autoMode,
        data.annual_rent
        // IMPORTANT: NOT including 'data' or 'onChange' to prevent infinite loops
    ]);
    ...
    const handleChange = (field, value) => {
        const updated = { 
            ...
            ...(['cam_reimbursements','other_reimbursements','other_income'].includes(field) && { [`${field}_auto`]: false })
        };
        onChange(updated);
        ...
        if (field === 'annual_rent') {
            ...
            lastCalculatedRentRef.current = null;
        }
    };

    const AutoField = ({ field, label, desc, autoDesc, value, isAuto, autoValue }) => (
        <div>
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                    <Label>{label}</Label>
                    <InfoTooltip content={isAuto ? autoDesc : desc} />
                </div>
                <div className="flex items-center gap-2">
                    <Switch
                        checked={isAuto}
                        onCheckedChange={(checked) => {
                            if (checked) {
                                onChange({ ...data, [field]: autoValue(), [`${field}_auto`]: true });
                            } else {
                                onChange({ ...data, [`${field}_auto`]: false });
                            }
                        }}
    ...
    const translations = {
        en: {
            ...
            cam_auto_desc: "Auto: 10% of annual rent",
            ...
            other_reimbursements_auto_desc: "Auto: 5% of annual rent",
            other_income: "Other Income (annual)",
            other_income_desc: "Parking, signage, or other revenue",
            other_income_auto_desc: "Auto: 2% of annual rent",
        ...
        hu: {
            ...
            cam_auto_desc: "Auto: éves bérleti díj 10%-a",
            ...
            other_reimbursements_auto_desc: "Auto: éves bérleti díj 5%-a",
            other_income: "Egyéb bevétel (éves)",
            other_income_desc: "Parkolás, reklám vagy más bevétel",
            other_income_auto_desc: "Auto: éves bérleti díj 2%-a",
        ...
        de: {
            ...
            cam_auto_desc: "Auto: 10% der Jahresmiete",
            ...
            other_reimbursements_auto_desc: "Auto: 5% der Jahresmiete",
            other_income: "Sonstige Einnahmen (jährlich)",
            other_income_desc: "Parkplatz, Werbung oder andere Einnahmen",
            other_income_auto_desc: "Auto: 2% der Jahresmiete",
