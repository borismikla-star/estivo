...
export default function CostInputs({ data, projectData, language, onChange }) {
  const translations = {
    sk: {
      ...
      auto_tooltip: "Automatický výpočet na základe predvolených hodnôt",
    ...
    en: {
      ...
      auto_tooltip: "Automatically calculate based on default values",
      ...
      <div className="border rounded-lg p-4 bg-muted/10">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <Label htmlFor={`${prefix}_auto`} className="text-sm text-muted-foreground flex items-center gap-1.5 cursor-pointer">
            ...
            <Switch 
              id={`${prefix}_auto`}
              checked={!isManual}
              onCheckedChange={(checked) => handleModeChange(prefix, !checked)}
            />
          </div>
        </div>
